...
(function consumeRecoveryHandoff() {
    ...
    if (!m) return;
    storeSession(m[1].toLowerCase());
    try { history.replaceState(null, '', window.location.pathname + window.location.search); } catch {}
})();

function isAuthed() {
    const s = getStoredSession();
    return !!(s && s.sessionToken && /^[a-f0-9]{64}$/i.test(s.sessionToken) && s.ts && (Date.now() - s.ts) < SESSION_TTL);
}

if (IS_SHARE_VIEW) document.documentElement.style.visibility = 'visible';
else if (!isAuthed()) document.documentElement.style.visibility = 'hidden';
...
function showPinOverlay() {
    ...
    <div id="pin-title">Joel Pagett's Tracker &amp; Tools</div><div id="pin-sub">Enter your PIN to continue</div>
    ...
    function updateDots() {
        for (let i = 0; i < 4; i++) document.getElementById('d'+i).className = 'pin-dot' + (i < entered.length ? ' filled' : '');
    }
    function showPinError(message) {
        ...
        setTimeout(() => { entered=''; busy=false; updateDots(); overlay.querySelectorAll('.pin-dot').forEach(d=>d.classList.remove('error')); el.classList.remove('show'); }, 1600);
    }
    async function checkPin() {
        ...
        try {
            ...
            if (!res.ok || !json.ok || !json.data || !json.data.session_token) throw new Error(json.error || `Authentication failed (HTTP ${res.status})`);
        ...
        } catch (err) { showPinError(err && err.message ? err.message : 'Authentication failed'); }
    }
    function addDigit(n) {
        if (busy || entered.length >= 4) return;
        entered += String(n); updateDots(); if (entered.length === 4) setTimeout(checkPin,50);
    }
    overlay.querySelectorAll('.pin-btn[data-n]').forEach(btn => btn.addEventListener('click', () => addDigit(btn.dataset.n)));
    document.getElementById('pin-del').addEventListener('click', () => { if (!busy) { entered=entered.slice(0,-1); updateDots(); } });
    document.addEventListener('keydown', function h(e) {
        ...
        else if (e.key==='Backspace' && !busy) { entered=entered.slice(0,-1); updateDots(); }
    });
}
...
if (IS_SHARE_VIEW) {
    ...
    document.documentElement.style.visibility='visible';
    window._mytripsAuthed=true;
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded',()=>document.dispatchEvent(new Event('mytrips:authed')),{once:true});
